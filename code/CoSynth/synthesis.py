...
class Synth(object):
    def __init__(self, words_ids_file, prosody_model_file, tacotron_model_file, waveglow_model_file, unit_dir):
        ...
        self.sigma = 1.0
        self.denoiser = Denoiser(self.waveglow, filter_length=hparams.n_fft, win_length=hparams.win_size).cuda()
        self.denoiser_strength = 1.0
    ...
    def synth(self, txts, logger='Synth'):
        ...
        if len(X) > 0:
            ...
            # [np.save('temp/%d.npy'%i, mel[:output_lengths[i]]) for i, mel in enumerate(mels)]
            mels = torch.from_numpy(mels.transpose((0, 2, 1))).cuda().half()
            with torch.no_grad():
                mwavs = self.waveglow.infer(mels, sigma=self.sigma)
                mwavs = self.denoiser(mwavs, self.denoiser_strength).squeeze(1).cpu().numpy()
